...
// ── Tarjeta de resumen por departamento ───────────────────────────────────────
function ResumenCard({ departamento, datos, tecnologia, color }) {
  ...
  const solar = filtrado
    .filter((d) => d.tecnología === "Solar")
    .reduce((s, d) => s + d.producción_mwh, 0);
  const eolica = filtrado
    .filter((d) => d.tecnología === "Eólica")
    .reduce((s, d) => s + d.producción_mwh, 0);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className={`badge ${domina === "Solar" ? "badge-amber" : "badge-purple"}`}
  ...
  const pivotado = useMemo(() => {
    ...
    datos
      ...
      .forEach(({ mes, departamento, producción_mwh }) => {
        ...
        // Acumular en lugar de pisar
        mapa[mes][departamento] =
          (mapa[mes][departamento] || 0) + producción_mwh;
      });
    ...
    <ResponsiveContainer width="100%" height={260}>
      ...
      >
        ...
        <Line
          type="monotone"
          dataKey={depto1}
          stroke={COLORES.depto1}
          strokeWidth={2.5}
          dot={{ r: 3, fill: COLORES.depto1, strokeWidth: 0 }}
          activeDot={{ r: 6 }}
        />
        <Line
          type="monotone"
          dataKey={depto2}
          stroke={COLORES.depto2}
          strokeWidth={2.5}
          strokeDasharray="6 3"
          dot={{ r: 3, fill: COLORES.depto2, strokeWidth: 0 }}
          activeDot={{ r: 6 }}
        />
      </LineChart>
    </ResponsiveContainer>
...
function GraficaBarrasComparativa({ datos, depto1, depto2, tecnologia }) {
  const pivotado = useMemo(() => {
    ...
    return techs.map((tech) => {
      const v1 = datos
        .filter((d) => d.departamento === depto1 && d.tecnología === tech)
        .reduce((s, d) => s + d.producción_mwh, 0);
      const v2 = datos
        .filter((d) => d.departamento === depto2 && d.tecnología === tech)
        .reduce((s, d) => s + d.producción_mwh, 0);
...
export default function Comparar({ mensual, porDepto }) {
  ...
  // Combinar mensual y porDepto para los resúmenes
    ...
    <section id="comparar">
      <div className="section">
        ...
        <p className="section-subtitle">
          Selecciona dos departamentos y un tipo de energía para comparar su
          producción lado a lado.
        </p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          {/* Líneas — evolución mensual comparada */}
          <div className="card">
            <p className="section-label">Evolución mensual</p>
            <h3
              style={{
                fontSize: "1rem",
                fontWeight: 700,
            ...
            >
              {depto1} <span style={{ color: "var(--text-muted)" }}>vs</span>{" "}
              {depto2} — {tecnologia}
            </h3>
            <GraficaComparativa
              datos={mensual}
          ...
          {/* Barras — totales por tecnología */}
          <div className="card">
            <p className="section-label">Totales acumulados</p>
            <h3
              style={{
                fontSize: "1rem",
                fontWeight: 700,
